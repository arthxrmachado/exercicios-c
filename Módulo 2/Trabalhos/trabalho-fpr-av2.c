/******************************************************************************
	...
	Data: 05/07/2021   -   Trabalho AV2
	
	Pede-se o desenvolvimento de um programa que manipule uma lista encadeada, 
	do tipo TLista, contendo números inteiros positivos, com a restrição de que 
	não podem existir elementos repetidos.
	
	A execução do programa deverá consistir na contínua disponibilização ao 
	usuário de um menu com possíveis operações, de forma que a execução só 
	terminará quando for selecionada a opção de saída.
	
	As operações que serão disponibilizadas ao usuário são:
	
	i. Inserção de um novo elemento;
	
	ii. Remoção de determinado valor (por exemplo, remover o número 10 da lista);
	
	iii. Remoção do elemento que encontra-se em dada posição (por exemplo, 
	remover o 5o elemento da lista);
	
	iv. Alteração de um valor por outro (por exemplo, alterar o no 10 pelo 
	número 8). Tenham cuidado para que esta operação não resulte em repetição 
	de elementos na lista;
	
	v. Busca de determinado elemento na lista, retornando a sua posição, se 
	encontrado, ou o valor NULL, caso contrário;
	
	vi. Exibição de todos os elementos da lista;
	
	vii. E, por último, a opção de sair do programa.
